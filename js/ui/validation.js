 ...
 * @author qijun.weiqj 
 */
define('ui.Validation', ['require', 'jQuery', 'Class', 'Log', 'ui.FormUtil'], 
...
var Validation = new Class({
	...
	init: function(elm, config) {
		...
		this.handler = Util.get('Handler', this.config.handler);
	...
	_createAdvice: function() {
		var self = this,
			advice = Util.get('Advice', this.config.advice);
		return {
			prompt: function() {
	...
	_validate: function(value, rule) {
		return Validation.validate(value, rule, this.elm);
	}
 ...
 */
$.extend(Validation, {
	validate: function(value, rule, data) {
		var type = rule.type || rule.validate,
			param = rule.value || rule.param;
		
		if (!type) {
			log.warn('empty validation type');
			return true;
		}
		...
		if (typeof type === 'string') {
			...
			v = Validator[type];
			if (!v) {
				log.warn('invalid validation type: ' + type);
				return true;
			}
		}

		return typeof type.test === 'function' ? type.test(value, param, data) :
			v.call(o, value, param, data);
	}
...
Validation.Validator = {
	...
	range: function(v, options) {
		return this._range(v, options, /^\d+(\.\d+)?$/);
	},

	'int-range': function(v, options) {
		return this._range(v, options, /^\d+$/);
	},

	_range: function(v, options, pattern) {
		if (!pattern.test(v)) {
			return false;
		}
